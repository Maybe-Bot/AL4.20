...
- `summary` writes lifecycle, checkpoint, periodic summary, and run-boundary
  records. Lifecycle records preserve every birth and death cause.
...
- `april_1`: The simulated date entered April 1.
- `invalid_state`: Neural or organism state failed host validation.
- `shutdown`: The experiment ended explicitly.

### `summary`

Records periodic population statistics. Fields include `population`,
`population_bytes`, `population_genome_parameters`, and cumulative births,
deaths, reproduction attempts, and mutations.
...
Records creation of the configured two-organism seed population after an
April 1 extinction and the transition to a permitted date.
...
1. April 1 deaths, if any, appear before any other organism event, and no
   execution-derived event appears for that date.
2. Reproduction attempts appear before their successful birth.
3. A birth appears before any capacity death it causes.
4. A death is the final event for that organism ID.
5. The periodic summary appears after births, deaths, and accounting for the
   tick.
6. A checkpoint event appears only after the checkpoint has been written
   successfully.

Buffered output can remain in process memory briefly. A crash can lose the most
recent buffer, but every flushed line is a complete record.
Use a different event-log and checkpoint path for each concurrently running
experiment. The files do not provide multi-process locking.

Use unique event-log and checkpoint paths for concurrent experiments. Writers
do not coordinate access to a shared path, and overlapping runs can corrupt or
replace one another's output.
...
```json
{"event":"run_start","log_version":1,"tick":0,"year":2000,"month":1,"day":1,"seed":420,"config_fingerprint":6759519941994754753,"resumed":false}
{"event":"birth","tick":0,"year":2000,"month":1,"day":1,"organism_id":1,"parent_a":0,"parent_b":0,"generation":0,"organism_bytes":8192,"mutations":0}
{"event":"summary","tick":1000,"year":2000,"month":1,"day":2,"population":17,"population_bytes":139264,"births":17,"deaths":0,"reproduction_attempts":31,"mutations":6}
